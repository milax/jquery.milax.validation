...
$fieldset);return valRes};var doFieldsValidation=function($fieldsToValidate){var isValid=true,validationClasses=new Array("mxRequired","mxEmail","mxNumber","mxMax","mxMin","mxAtLeastOne");$fieldsToValidate.each(function(){var $input=$(this);$input.closest(options.fieldHolderSelector).find(options.errMsgClass).hide();$input.removeClass(options.fieldHasErrorClass.substr(1));for(var i in validationClasses){var currClass=validationClasses[i];if($input.hasClass(currClass)&&!isFieldValid(currClass,$input)){isValid=
false;return true}}});return isValid};var isFieldValid=function(way,$what2Validate){var val=$.trim($what2Validate.val()),opts=$what2Validate.data("mxvalidateOptions"),$errMsg=$what2Validate.closest(options.fieldHolderSelector).find(options.errMsgClass+"."+way),isValid=true,pattern;switch(way){case "mxRequired":isValid=!!val;break;case "mxEmail":pattern=/^[_a-z0-9-]+(\.[_a-z0-9-]+)*@[a-z0-9-]+(\.[a-z0-9-]+)*(\.[a-z]{2,4})$/;isValid=!(val.match(pattern)===null);break;case "mxNumber":pattern=/^\d*$/;
isValid=!(val.match(pattern)===null);break;case "mxMax":var maxLength=opts.max;isValid=!(val.length>maxLength);break;case "mxMin":var minLength=opts.min;isValid=!(val.length<minLength);break;case "mxAtLeastOne":var $checkedCheckboxes=$what2Validate.find('input[type="checkbox"]:checked');isValid=!($checkedCheckboxes.length===0);break;default:isValid=false}if(!isValid)showErrorMsg($errMsg,$what2Validate);return isValid};var showErrorMsg=function($errorMsg,$what2Validate){$errorMsg.css({display:"block"});
$what2Validate.addClass(options.fieldHasErrorClass.substr(1))};this.init()}})(jQuery);